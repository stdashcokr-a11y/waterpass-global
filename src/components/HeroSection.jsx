...
export default function HeroSection() {
  ...
  const t = {
    ...
    hero_title: language === 'en' ? (
      <>ENGINEERING THE<br />FUTURE OF<br />URBAN WATER<br />MANAGEMENT</>
    ) : (
      <>도시 수자원<br />관리의 미래를<br />엔지니어링<br />하다</>
    ),
    hero_sub: language === 'en' ? 'Next Generation Permeable Pavement Technology' : '차세대 투수성 보도블록 기술'
  };

  return (
    <section className="relative w-full h-screen min-h-[800px] flex flex-col items-center justify-center overflow-hidden bg-[#020813]">
      ...
      <div className="absolute inset-0 z-0">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#050D1D]/80 to-[#050D1D]" />
      ...
      {/* Navigation Bar */}
      <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[#050D1D]/90 backdrop-blur-md border-b border-white/10 py-4' : 'bg-transparent py-6'}`}>
        <div className="max-w-[1600px] mx-auto px-4 md:px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4 relative z-50 flex-wrap sm:flex-nowrap justify-end">
            ...
            <a 
              href="https://www.waterpass.kr" 
              target="_blank" 
              rel="noopener noreferrer"
              className="group relative inline-flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-4 py-1 sm:py-1.5 ml-2 sm:ml-8 bg-gradient-to-r from-red-600 to-[#ff3b3b] text-white text-[8px] sm:text-[9px] whitespace-nowrap font-black tracking-widest flex-shrink-0 rounded-full overflow-hidden shadow-[0_0_15px_rgba(255,0,0,0.4)] hover:shadow-[0_0_25px_rgba(255,0,0,0.6)] transition-all hover:scale-105 active:scale-95"
            >
              ...
              <span className="relative flex items-center gap-1 sm:gap-2 whitespace-nowrap">
                {t.compare}
                <ExternalLink className="w-2 sm:w-3 h-2 sm:h-3 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
              </span>
            </a>
      ...
      <div className="relative z-10 text-center max-w-5xl px-4 mt-20 md:mt-0">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6 w-full max-w-[280px] sm:max-w-none mx-auto">
          <button className="w-full sm:w-auto px-8 py-3 sm:px-12 sm:py-4 bg-[#00AEEF] text-black font-black text-xs sm:text-sm tracking-widest uppercase rounded-sm hover:scale-105 transition-transform shadow-[0_0_20px_rgba(0,174,239,0.4)]">
            {t.nav_tech}
          </button>
          <button className="w-full sm:w-auto px-8 py-3 sm:px-12 sm:py-4 bg-transparent border border-white/20 text-white font-bold text-xs sm:text-sm tracking-widest uppercase rounded-sm hover:bg-white/10 transition-colors backdrop-blur-sm">
            {t.nav_contact}
          </button>
        </div>
      </div>
